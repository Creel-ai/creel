...
class TestQuietHours:
    ...
    def test_disabled_config(self):
        ...
        with patch("taskrunner.quiet_hours.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 1, 0, tzinfo=ZoneInfo("UTC")
            )  # 01:00 UTC
    ...
    def test_within_quiet_hours_same_day(self):
        ...
        with patch("taskrunner.quiet_hours.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 9, 0, tzinfo=ZoneInfo("UTC")
            )  # 09:00 UTC
            assert is_quiet_hours(config) is True

            # Test at end boundary (exclusive)
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 17, 0, tzinfo=ZoneInfo("UTC")
            )  # 17:00 UTC
            assert is_quiet_hours(config) is False
    ...
    def test_outside_quiet_hours_same_day(self):
        ...
        with patch("taskrunner.quiet_hours.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 18, 0, tzinfo=ZoneInfo("UTC")
            )  # 18:00 UTC
            assert is_quiet_hours(config) is False
            assert should_suppress(config) is False
    ...
    def test_midnight_crossing_within_quiet_hours(self):
        ...
        with patch("taskrunner.quiet_hours.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 23, 30, tzinfo=ZoneInfo("UTC")
            )  # 23:30 UTC
            assert is_quiet_hours(config) is True
            assert should_suppress(config) is True
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 7, 0, tzinfo=ZoneInfo("UTC")
            )  # 07:00 UTC
            assert is_quiet_hours(config) is True
            assert should_suppress(config) is True
            ...
            # Test at start boundary (inclusive)
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 23, 0, tzinfo=ZoneInfo("UTC")
            )  # 23:00 UTC
            assert is_quiet_hours(config) is True
    ...
    def test_midnight_crossing_outside_quiet_hours(self):
        ...
        with patch("taskrunner.quiet_hours.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 15, 0, tzinfo=ZoneInfo("UTC")
            )  # 15:00 UTC
            assert is_quiet_hours(config) is False
            assert should_suppress(config) is False
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 22, 0, tzinfo=ZoneInfo("UTC")
            )  # 22:00 UTC
            assert is_quiet_hours(config) is False
            assert should_suppress(config) is False
    ...
    def test_urgent_bypass(self):
        ...
        config = QuietHoursConfig(
            enabled=True, start="23:00", end="08:00", timezone="UTC", allow_urgent=True
        )
    ...
    def test_urgent_disabled(self):
        ...
        config = QuietHoursConfig(
            enabled=True, start="23:00", end="08:00", timezone="UTC", allow_urgent=False
        )
    ...
    def test_timezone_handling(self):
        ...
        with patch("taskrunner.quiet_hours.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = datetime(
                2024, 1, 1, 23, 0, tzinfo=ZoneInfo("America/Denver")
            )
            assert is_quiet_hours(config) is True
    ...
    def test_invalid_timezone(self):
        ...
        config = QuietHoursConfig(
            enabled=True, start="23:00", end="08:00", timezone="Invalid/Timezone"
        )
